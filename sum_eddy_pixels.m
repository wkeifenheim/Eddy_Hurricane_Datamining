...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% WARNING: loading eddy timeslices will take several hours
...
%running index of what hurricane track we are on. should not exceed 1911
j = 1;
...
cur_hur = IBTrACS_1992_2010_daily.Serial_Num(1);
wait_h = waitbar(0,'progress');

for i = 1 : 60819
    
    %advance the hurricane counter if needed..
    if(~strcmp(cur_hur, IBTrACS_1992_2010_daily.Serial_Num(i)))
        j = j + 1;
        cur_hur = IBTrACS_1992_2010_daily.Serial_Num(i);
        rem_ed_type = NaN;
        rem_ed_idx = NaN;
        rem_ed_count = 0;
        first = 1;
    ...
    time_idx = find(eddy_track_date_indices.Date(:) == IBTrACS_1992_2010_daily.TimeSlice(i));
    e_idx = IBTrACS_1992_2010_daily.EddyIdx(i);
    
    if(IBTrACS_1992_2010_daily.EddyClass(i) == -1) %cyc
        %HHH:E:PPPP
        hhh = num2str(IBTrACS_1992_2010_daily.HurricaneAge(i));
        e = 'C';
...
%         list{j} = strcat(list{j}, {' '}, list_str);
        ...
        rem_ed_type = -1;
        rem_ed_idx = IBTrACS_1992_2010_daily.EddyIdx(i);
        if(first)
    ...
    elseif(IBTrACS_1992_2010_daily.EddyClass(i) == 1) %acyc
        hhh = num2str(IBTrACS_1992_2010_daily.HurricaneAge(i));
        e = 'A';
